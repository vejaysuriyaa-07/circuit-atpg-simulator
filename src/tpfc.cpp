#include "../includes/core/tpfc.hpp"
...
int testPatternCoverage_impl(Simulator &simulator) {
  ...
  if (!(argStream >> tpCount >> freq >> outputPath >> reportFilePath)) {
    printf("Function usage: TPFC <tp_count> <frequency> <output_file> "
           "<report_file>\n");
    return 0;
  }
  const std::string resultsDir = "../tpfc_results";
  ...
  auto ensureCleanDir = [](const std::string &dir) -> bool {
    ...
    if (stat(dir.c_str(), &sb) != 0) {
      if (mkdir(dir.c_str(), 0755) != 0) {
        printf("TPFC: Unable to create results directory: %s\n", dir.c_str());
        return false;
      }
    } else if (!S_ISDIR(sb.st_mode)) {
      printf("TPFC: Results path is not a directory: %s\n", dir.c_str());
      return false;
    }

    DIR *dirStream = opendir(dir.c_str());
    if (!dirStream) {
      printf("TPFC: Unable to open results directory: %s\n", dir.c_str());
      return false;
    }
  ...
  const std::string tpFile = resultsDirWithSlash + "tpfc_tp.txt";
  ...
  std::string rtpgArgs = std::to_string(tpCount) + " b " + tpFile;
  std::string rflArgs = rflFile;
  std::string dfsArgs = tpFile + " " + dfsFile;

  simulator.setCommandArgs(rtpgArgs);
  simulator.randomTestPatternGen();
  ...
  if (totalFaults == 0) {
    printf("TPFC: Reduced fault list is empty or missing, skipping coverage "
           "report generation.\n");
    return 0;
  }
  ...
  if (stepFiles.empty()) {
    printf("TPFC: No DFS step files found, skipping coverage report "
           "generation.\n");
    return 0;
  }
  ...
  if (!reportFile.is_open()) {
    printf("TPFC: Unable to open report file: %s\n", reportFullPath.c_str());
    return 0;
  }
  ...
  for (std::size_t idx = 0; idx < stepFiles.size(); ++idx) {
    ...
    if (!stepFile.is_open()) {
      printf("TPFC: Unable to open step file: %s\n", path.c_str());
      reportFile << stepNum << ' '
  ...
  printf("TPFC: Final coverage: %.6f (%zu/%zu faults)\n", finalCoverage,
         detectedFaults.size(), totalFaults);
  printf("TPFC: Time elapsed: %.3f seconds\n", elapsedSeconds);
  ...
  if (source.is_open()) {
    ...
    if (!destination.is_open()) {
      printf("TPFC: Failed to open final output file '%s' for writing.\n",
             outputFullPath.c_str());
    } else {
      destination << source.rdbuf();
    }
  } else {
    printf("TPFC: DFS output file '%s' not found; coverage report only.\n",
           dfsFile.c_str());
  }
  return 1;
}
